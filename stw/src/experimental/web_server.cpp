...
namespace stw::experimental
{
    ...
    int web_server::run(const std::string &bindAddress, uint16_t port, uint32_t backlog)
    {
        if (isRunning.load())
            return 1;
        ...
        {
            workers.push_back(std::make_unique<worker_context>());
            workers.back()->thread = std::thread(&web_server::network_loop, this, workers.back().get());
        }
    ...
    void web_server::on_read(worker_context *worker, int32_t fd)
    {
	...
	void web_server::process_request(worker_context *worker, std::shared_ptr<http_context> context, std::shared_ptr<network_stream> networkStream)
	{
    ...
    void web_server::finalize_request(worker_context* worker, std::shared_ptr<http_context> context) 
    {
	...
	void web_server::send_response(worker_context *worker, std::shared_ptr<http_context> context, uint32_t statusCode)
	{
		worker->poller->remove(context->connection->get_file_descriptor());
    ...
    void web_server::on_write(worker_context *worker, int32_t fd)
    {
        auto it = worker->contexts.find(fd);
    ...
    void worker_context::remove(std::shared_ptr<http_context> context, const char *sender)
    {
        if(!context.get())
            return;
